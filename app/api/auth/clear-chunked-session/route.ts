...
export async function POST(request: NextRequest) {
  try {
    const response = NextResponse.json({ 
      success: true, 
      message: "Chunked session tokens cleared successfully" 
    })
    
    // Clear ALL chunked session tokens (0-99) that are causing the issue
    for (let i = 0; i < 100; i++) {
    ...
    console.error("Error clearing chunked session:", error)
    return NextResponse.json(
      { error: "Failed to clear chunked session" },
      { status: 500 }
    )
  }
}
