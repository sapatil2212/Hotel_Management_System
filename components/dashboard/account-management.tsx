...
<<<<<<< HEAD
                      ...
                      <TableCell>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleViewCredit(transaction)}
        ...
        <DialogContent className="max-w-2xl">
          ...
          {selectedTransaction && (
            <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900">
                        {format(new Date(selectedTransaction.transactionDate), 'MMM dd, yyyy HH:mm:ss')}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Type</Label>
                      <Badge className={`${getTransactionTypeColor(selectedTransaction.type)} text-xs px-2 py-1`}>
                        {selectedTransaction.type.toUpperCase()}
                      </Badge>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Category</Label>
                      <p className="text-sm text-gray-900">{getCategoryDisplayName(selectedTransaction.category)}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border border-gray-200 shadow-sm">
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg font-semibold text-gray-900">Financial Details</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Amount</Label>
                      <p className={`text-2xl font-bold ${getTransactionTypeColor(selectedTransaction.type)}`}>
                        {selectedTransaction.type === 'credit' ? '+' : '-'}{formatCurrency(selectedTransaction.amount)}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Account</Label>
                      <p className="text-sm text-gray-900">{selectedTransaction.account.accountName}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Payment Method</Label>
                      <p className="text-sm text-gray-900">
                        {selectedTransaction.paymentMethod ? selectedTransaction.paymentMethod.replace('_', ' ').toUpperCase() : 'N/A'}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Processed By</Label>
                      <p className="text-sm text-gray-900">{selectedTransaction.processedBy || 'System'}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border border-gray-200 shadow-sm">
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg font-semibold text-gray-900">Reference Information</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900">
                        {selectedTransaction.referenceType ? selectedTransaction.referenceType.replace('_', ' ').toUpperCase() : 'N/A'}
                      </p>
                    </div>
                  </div>
                  <div>
                ...
                <Card className="border border-green-200 bg-green-50 shadow-sm">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg font-semibold text-green-900 flex items-center gap-2">
                      ...
                      Revenue Credit Information
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div className="text-sm text-green-800">
                      <p><strong>✓ This transaction represents consolidated revenue earned by the hotel</strong></p>
                      <p>• Automatically added to the Hotel account balance</p>
                      <p>• Includes all revenue components (accommodation, extra charges, taxes)</p>
                      <p>• Contributes to the overall financial performance</p>
                      <p>• Tracked for revenue reporting and analysis</p>
                    </div>
                    
                    {/* Revenue Breakdown Display */}
                    {selectedTransaction.notes && selectedTransaction.notes.includes('Revenue Breakdown:') && (
                    ...
                    <div className="mt-3 p-3 bg-white rounded border border-green-200">
                      <p className="text-xs text-green-700">
                        <strong>Revenue Category:</strong> {getCategoryDisplayName(selectedTransaction.category)}
                      </p>
                      <p className="text-xs text-green-700">
                        <strong>Revenue Source:</strong> {selectedTransaction.referenceType ? selectedTransaction.referenceType.replace('_', ' ') : 'Direct'}
                      </p>
                      <p className="text-xs text-green-700">
                        <strong>Total Revenue:</strong> {formatCurrency(selectedTransaction.amount)}
                      </p>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="flex justify-end gap-2">
                ...
                {selectedTransaction.type === 'credit' && (
                  <Button 
                    onClick={() => {
                      // Handle export or additional actions
